...
/**
 * Only allow block nodes or inline and text nodes in blocks, but not a mix.
 */

function blocksContainBlocksOrInlines(node) {
...
/*
 * Map every rules to the object they apply to. Rules are order-sensitive
 */
...
/**
 * Combine all core schema rules into one
 */

function coreSchemaRule(node) {
  const rules = CORE_SCHEMA[node.object] || EMPTY
